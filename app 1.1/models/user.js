...
var sql = "update user set username=?, phone=? where id=?";
		...
		eupdate: function(user, callback){
var sql = "update user set username=?, password=?, phone=?, picture=? where id=?";
				
		db.execute(sql, [user.username, user.password, user.phone, user.filename, user.id], function(status){
			if(status){
			console.log(status);
				callback(true);
			}else{
